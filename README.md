...
### Рекомендации по скорости устранения уязвимостей.
...
Из перечисленного выше топа критических уязвимостей, три из них успользуются для сетевых атак, т.е. эксплуатация осуществляется путем отправления запроса на какой-либо порт.
...
Цель этого эксплойта состоит в том, чтобы создать RCE (удаленное выполнение кода), которое мы могли бы затем использовать для создания обратной оболочки.
